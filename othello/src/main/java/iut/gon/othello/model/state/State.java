...
/**
 * Représentation concrète et immuable de l'état du jeu (record).
 * Encapsule la disposition actuelle du plateau, l'équipe dont c'est le tour,
 * et les éventuelles lignes de pions formées qui attendent d'être retirées.
 *
 * @param board La carte représentant le plateau (coordonnées vers jetons).
 * @param turn  L'équipe qui doit jouer le prochain coup.
 * @param lines La liste des lignes complètes détectées sur le plateau.
 */
public record State (Map<Coordinate, Token> board, Team turn, List<Set<Coordinate>> lines) implements IState {
    
    /**
     * Applique un mouvement sur le plateau. Cette méthode vérifie la validité du déplacement,
     * déplace l'anneau, dépose un pion sur la case de départ, retourne les pions adverses 
     * survolés et met à jour l'état des lignes et du tour.
     *
     * @param move L'action de déplacement à appliquer.
     * @return Un nouvel état {@link State} représentant le jeu après le déplacement.
     * @throws IndexOutOfBoundsException Si les coordonnées de départ ou d'arrivée sont hors terrain.
     * @throws RuntimeException Si une ligne doit d'abord être supprimée avant de pouvoir bouger.
     * @throws IllegalArgumentException Si le mouvement est invalide (mauvais anneau ou destination inatteignable).
     */
    @Override
    ...
    /**
     * Applique une action de suppression de ligne. Retire les pions de la ligne spécifiée 
     * ainsi que l'anneau choisi, puis met à jour le tour de jeu si aucune autre ligne ne reste.
     *
     * @param rm L'action contenant la ligne et l'anneau à retirer.
     * @return Un nouvel état {@link State} mis à jour.
     * @throws IndexOutOfBoundsException Si l'anneau ou une coordonnée de la ligne est hors terrain.
     * @throws RuntimeException S'il n'y a aucune ligne à supprimer.
     * @throws IllegalArgumentException Si la ligne est invalide ou si l'anneau n'appartient pas au joueur.
     */
    @Override
    ...
    /**
     * Calcule tous les mouvements légaux possibles pour un anneau situé à une coordonnée donnée.
     * Un anneau peut se déplacer en ligne droite dans les 6 directions, en glissant sur des cases vides 
     * ou en sautant par-dessus des pions, mais ne peut pas sauter un autre anneau.
     *
     * @param from La coordonnée de l'anneau à déplacer.
     * @return Un ensemble des coordonnées d'arrivée possibles.
     */
    @Override
    ...
    /**
     * @return Une carte classant la position des anneaux par équipe.
     */
    @Override
    ...
    /**
     * Évalue les conditions de victoire. Une équipe gagne lorsqu'elle retire 
     * suffisamment d'anneaux du plateau (généralement 3 retirés, donc 2 restants).
     *
     * @return Le gagnant ({@link Team}), ou {@code null} si la partie continue.
     */
    @Override
    public Team winner() {
    ...
    /**
     * Vide la case spécifiée en retirant le jeton qui s'y trouve.
     *
    ...
    /**
     * Ajoute, remplace ou supprime un jeton de manière dynamique à une coordonnée précise.
     * Si le jeton cible est identique à celui déjà présent, la case est vidée (effet de bascule).
     * Particulièrement utile pour les outils de création/édition de niveau.
     *
     * @param position   La coordonnée cible.
     * @param team       L'équipe du jeton à placer.
     * @param tokenClass La classe du jeton à instancier (ex: Ring.class, Pawn.class).
     * @return Un nouvel état {@link State} reflétant les modifications.
     * @throws IndexOutOfBoundsException Si la coordonnée est hors du terrain.
     * @throws RuntimeException En cas d'erreur de réflexion lors de l'instanciation du jeton.
     */
    @Override
    ...
    /**
     * Vérifie si une coordonnée donnée fait bien partie du plateau de jeu.
     *
     * @param c La coordonnée à vérifier.
     * @return {@code true} si la case existe sur le plateau, {@code false} sinon.
     */
